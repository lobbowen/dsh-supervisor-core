...
18. **run `35489972031`（HEAD `931d734`）—— 第 4 批首次四平台全绿，§H-7-17 的「以下一个 run 为准」兑现**：
    ...
    - **合入与合入后复绿**：head `f0aaa76` 三次连续全绿后，PR #6 于 2026-09-20 以 **merge commit `c649c08`**
      合入 master；master push-run `35490996571` 再次四平台全绿，合入后的纯文档收尾直推（`4ea2df3`）
      也由 run `35491330422` 复绿确认（`release` 按非 tag 规则 skipped）。
...
### I-13 第五轮（run 35500803270）：本批裁决绿 + 合入，回显把「慢」与「卡死」分开了

1. head `292f644` 上 precheck、Linux test job、四平台 build 腿全部 success，release 按 PR 语义 skipped；
   整个日志包硬 FAIL 行为 0。test 腿日志从 90 行（截断后）变成 3950 行，说明上一项去截断生效。
2. 回显第一次给出真实分布：`P9 反代账号已进入视图 耗时=2510ms 轮询=11 状态=["frozen"]`、
   `B3 反代账号注册完成 耗时=2257ms 轮询=10 状态="ready"`。即正常负载下到终态只需 2~3 秒，
   第三轮那次越过 20s 是 runner 拥塞下的尾部，而非产品常态——正是「无回显时无法把两者区分开」的代价。
   也据此确认 60s 上限作为失控守卫留了 20 倍以上余量，不需要再放宽。
3. 合入：PR #7 以 `{"sha": 292f644…}` 锁定被裁决的 head 合入（merge commit `6aac421`），
   与分支当时的 head 同值，排除了「裁决后又有新提交」的错位。
