...
- **验证口径**：本机**只允许** `node --check` / `bash -n` 与纯静态源码形态门禁（读文件+正则，不执行行为测试）；**测试套件/全量链一律不得在本机运行**——运行时验证唯一裁判是 CI 四平台矩阵（本机绿灯对 win/mac 语义零证明力，且会形成心理放行沉淀平台性债务）。违例勘误见 §G-6-8。
...
8. **违例勘误（2026-09-20，用户裁定）**：本批提交前曾在本机多遍运行全量链与行为探针——违反「运行时测试仅 CI 四平台矩阵裁决」口径（本机不具备 win/mac 语义，绿灯无证据力）。副作用已收口：脱离会话的 `dry-run-proxy` 孤儿进程已回收、`/tmp` 测试产物已清理、仓库工作区无残留。教训固化：本机检查止于 `node --check`/`bash -n`/纯静态门禁；第 7 条三处发现的修复保留（均为 CI 会同样判红的真缺陷），但**不得**以「本地已全绿」作为任何合入依据——最终裁决只看 PR #5 的 CI 矩阵。
