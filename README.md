...
GET  /env/environment/last   上一拍快照的只读回看（{available,path,at,ageMs,reason,data}）：整份上一拍表单原样交回，
      没写过（never-written）与读不出/版本不符（unreadable-or-schema-mismatch）分开标；
      只读文件、零摸网零写盘、仅同源防护，**绝不参与分发判定**（分发永远走当场装配的那一份）
# 实例管理（沙箱）
