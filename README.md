...
| [GUARD-DOMAIN-MODEL.md](GUARD-DOMAIN-MODEL.md) | **规范（唯一事实源）** | **守护域模型唯一事实源**：两域（域 A 用户意图 / 域 B 基础设施）+ 铁律 G-1..G-6；§6 应然写权（目录四铁律 M-1..M-4、`desired` 唯一写口与沙箱意图落点 `inst.state.desired`、生命周期视图直写违例基线）；基础设施保活无守护计数（`guardian_action` 事件已随其唯一生产者 `_guardianEvent` 删除）。由 `test/guard-domain-model-gate-test.js` 机器校验（GD-1..GD-8 + ML-2/ML-3 ratchet）、`test/app-ctor-injection-test.js`（ML-1）|
