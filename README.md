...
| [RELEASE-CHANNEL-CONTRACT.md](RELEASE-CHANNEL-CONTRACT.md) | **规范（唯一事实源）** | **发布通道与选版唯一事实源**：canary/beta/rc/latest/rollback 五通道；选版算法冻结；紧急回退用显式 rollback 标签（铁律 `RC-*` 全集见该文件 §3「关键不变量」表，**本索引不抄编号与条数**）。由该文件 §6 表逐条指到执行位点（内核侧门禁在 `test/release-channel-gate-test.js`，壳侧两条在壳仓自己的单测里） |
...
| [GUARD-DOMAIN-MODEL.md](GUARD-DOMAIN-MODEL.md) | **规范（唯一事实源）** | **守护域模型唯一事实源**：两域（域 A 用户意图 / 域 B 基础设施）+ 铁律 G-1..G-6；§6 应然写权（目录四铁律 M-1..M-4、`desired` 唯一写口与沙箱意图无第二落点（B2-1 废止 `inst.state.desired`）、生命周期视图直写违例基线）；基础设施保活无守护计数（`guardian_action` 事件已随其唯一生产者 `_guardianEvent` 删除）。由 `test/guard-domain-model-gate-test.js` 机器校验（GD-1..GD-8 + ML-2/ML-3 ratchet）、`test/app-ctor-injection-test.js`（ML-1）|
...
GET  /env/environment        环境表单（schema 2 维度台账：本机实况按维度收齐——运行时 node/npm/镜像源/前缀、
      ...
      用户偏好、这一拍的分发依据 pick，每维带 at/source/state 与留痕 probed；未刷新的维度如实 pending）；
