...
- **CI 裁决补录（五轮红 + 一次崩溃，取证见 §H-7-5/6/9/11/12/13）**：在途 npm 的中止改走平台层整树终止
  （Windows 无进程组语义，旧 `process.kill(-pid)` 只杀得到 `npm.cmd` 壳，孙进程照旧写盘）；
  测试夹具侧修八处「判据/夹具自身失效」——D-12 反向例期望倒置、D-11 权限位缺 win32 门控、
...
- **链推进的正向证据**：#78–#96 共 19 个门禁文件首次被四平台执行并全绿（frp 单源写盘三段判据、
  glibc E-2 跨平台静态断言等）；链位 #113 的 API 重绑异步夹具在预清阶段以本机探针复现「同步读异步事实」
