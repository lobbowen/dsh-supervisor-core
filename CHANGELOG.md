...
### 接管归属判据不再按「命令行出现过 dsh」放行：守卫误杀无关进程收口

真机风险而非测试噪声。`src/app/main/signals.js#_isManagedProcess` 的兜底分支单靠 `isDshCmdline`
（整条命令行的 `/dsh/i` 子串匹配）放行接管，于是任何**路径里**含 `dsh` 的无关进程，只要恰好监听
target 端口就被守卫判成受管 DSH 并接管，接管后按崩溃换代对它发 SIGTERM —— 正是该函数注释声明要
避开的「路径碰巧含 dsh 误接管」。本批收口为「`web` 子命令词与 `dsh` 特征同时成立」，与
`src/app/main/port-rederive.js` 的 `genericDsh` 共用同一条闸：跨进程归属判定只此一处标准。
配置 bin 命中路径、以及手动标准安装的 DSH（`dsh web --port N`，路径与配置 bin 无关）都不受影响。
判据在 `adopt-token-reclaim-test` D-12：真实子进程双向对照（带 `web` 必放行 / 只含 `dsh` 字样必否决），
并断言反例命令行确实含 `dsh`，防判据空转。

发现路径本身值得记：`test/smoke.js` 的端口占用用例（不硬抢、只告警）在四个宿主上同一位置被子进程
SIGTERM 打死，而旧执行形态跑同一提交全过 —— 差异只在测试执行器把**绝对路径**写进了被测进程的命令行。
故同步立一条 harness 纪律：`test/_runner.js` 注入 `_preload` 一律用相对字面量，执行器不得把自己的
路径泄进被测对象可观测的输入里；`state-root-test` SR-7 相应从「钉实现写法」改为钉不变量，并补三条
反向样本（绝对路径形态 / 丢 `-r` 注入 / 回到 `&&` 巨链，都必判红）。

### 测试链改为分层登记表执行：门禁不再锁住开发，跨平台证据按宿主取

1. `scripts.test` 原为 `node -r ./test/_preload.js <file>` 的 `&&` 串联，实测链长 7991 字符、自设上限
   8000 —— Windows `cmd.exe` 的 8191 命令行上限被旧判据钉成硬约束，余量只剩 9 个字符，新增任何一道
   测试都要先挤掉一个旧条目，门禁反过来锁住了开发。唯一事实源现为 `test/manifest.js`（每条登记
   `file / tier / os / why`），入册成本降为「加一行」。
2. 分层执行：`test` job 把全量链判一遍；四条 build 矩阵腿只跑该宿主的 L2（真 spawn 进程、跑
   bash/pkill、读 `/proc`、断言文件权限位那批）。此前同一条链每次 push 被执行 5 遍，重复的是
   **平台无关**判据，不产生额外跨平台证据。宿主不在 `os` 列内时条目打 **SKIP 台账**（SKIP ≠ 通过）。
3. `test/_runner.js` 默认跑完全部条目、末尾汇总所有红点（`--fail-fast` 才短路），并记录 `signal=` /
   `error=`：只报「exit -1」等于没报。旧流程「首个红点截断其后全部条目，再按链中位置人工复算」作废。
4. 门禁 `test-chain-completeness` 重写为 C-a..C-j，其中两条是新能红的：C-i 要求碰宿主状态的测试
   必须有代码守卫（首跑真命中 2 处 —— 登录自启条目与能力审计会真改本机自启项，补 `test/_sandbox-guard.js`
   而非放宽判据）；C-j 禁止文档 / workflow 注释 / 登记表头把链条目数写死，数字只由该门禁实跑打印。
5. 消费方同步：`no-cross-repo` X-4、`release-spec` P-5、`state-root` SR-7、`acceptance-standard` A-2
   （新增「矩阵腿按 L2 分层跑回归」执法，防止退化成全量重复）。

跨平台是交付底线，本批一条测试都没有删。
## [0.1.6-BETA.17]（2026-09-26）
...
- **`platform-layer-portability` X-11 删去与运行判据重复的源码级正则**（端点三档透传、来源闸、契约登记计数、
  面板分档/选路/证据渲染的文案钉，连同其反向样本共二十余条）。这些判据所防的形态全部可由真实 HTTP 夹具与面板
  vitest 按行为判红：api-contract 的 OW/OU/EF 组从 socket 打到响应体，`externalOpen.test.ts` 按字段分档并验
  err.body 活着走到呈现口。源码正则的漏法（改名、字符串拼调用、间接 require）本来就不构成额外保障，而正则自身
  会随任何无害重构假红——重复即负债。
- **X-11 保留的是运行期看不见的那几件**：全仓单一打开动词（第二出口一旦出现，行为用例只会各测自己那条路）、
  平台事实与图形会话探测各处只许一份（副本漂移不改变单次行为，只在两处答案不一致时才暴露）、网关缺省装配
  （注入位没有缺省即留第二出口）、面板 window.open 唯一出口与四个消费页走统一入口。
- **随删补齐运行判据**：`round13-router-relay-gaps` 的 OAuth 轮次块新增一键登录成功/失败两档原样透传断言（confirmed、
  isolated、reason/url/evidence 均由平台层夹具决定，本层自宣称即红）；router-ops 的 isolated-login 意图接线
  是域与平台之间唯一一行 glue，删掉即登录静默换回普通窗口且运行面上看不出来，故文本判据保留并只留这一处；
  `api-contract` EF 组新增已认证非回环来源读表单 200 一例——只读面与动作面的来源闸不同级，此前这一不对称
  只有被删的正向正则撑着。`token-contract-gate` TK-G6（令牌不得进 spawn argv）原样保留：它守的是安全事实，
  同机 `ps` 能否看见不在任何运行路径上可观测。四平台 CI 矩阵为唯一裁判。

## [0.1.6-BETA.15]（2026-09-26）

### 壳观测上报的内核接收口：同一批事实有两个采集者时，并排而不是覆盖

- **新增 P7 文件契约 `supervisor/shell-report.json`（schema 1）的读取口** `src/platform/contract/shell-report.js`：
  壳把自己实跑到的 node/npm（`args` 与 program 成对）、全局前缀可写性、镜像源择优结果与逐条探测明细投进这个
  文件，内核只读、永不参与 spawn。选文件而不是新开上报端点的理由写在该文件头部：壳与内核恒同机，采集者就是写入者，
  走 HTTP 需要新造带体动词并加投递重试，而那条重试正是把「没送达」伪装成「已上报」。写入者在壳仓
  `src-tauri/src/shell_report.rs`（自桌面壳 1.2.10 起投放，`SCHEMA` 与本侧 `SUPPORTED_SCHEMA` 钉同值）；
  没装到该版本的机器上面板如实显示「这台机器的壳还没报过」，不猜、不补、不重试。
- **三档读回各由真实文件状态驱动**：`ok` / `never-written` / `unreadable-or-schema-mismatch`。缺 `at`、零观察、
  schema 不符、超字节上限、落点成了目录，一律落到「读不出」而不是「没有」——把后者说成前者会引着人去点刷新，
  反过来会把故障过成常态。读取口**永不抛错**。
- **环境表单新增 `shell` 维并与 `runtime` 维并排**（`SECTION_ORDER` 九维升十维）：`runtime` 是「本进程现在解析到的」，
  `shell` 是「装内核时真正用的那一套」。不做「壳覆盖内核同维字段」的取舍模型——覆盖会让「探针的 npm 与装内核的 npm
  不是同一个」这类缺陷重新隐身，两份读数的差异本身就是要上屏的证据。面板据此逐行对照（Node/npm/镜像源/前缀）并显示
  报告新鲜度与明细行数。
- **脱敏抽出为 `src/platform/util/redact.js`**：入站路径从一条变两条（自己的探针读数、别的进程写的报告），
  尺继续只有一把；`environment.js` 改为消费该模块并停止导出同名函数。
- 回归：`runtime-contract` 新增 SR 组（三档分档、schema handshake、三态不折叠、入站脱敏、明细超限只截断并如实报数、
  读取口永不抛、`shell-report.json` 路径口径全仓唯一）；`platform-layer-portability` X-13 新增两条可红判据
  （脱敏尺唯一：全仓只允许一处写替换串；`shell` 维进台账且其落点等于契约拼出来的那个路径）；`api-contract` EF 组的
  维度名单改为**从内核 `SECTION_ORDER` 取分母**并新增壳上报维的字段透传判据（在测试里再抄一份名单，等于给
  「新维度悄悄成为摆设」留门）。四平台 CI 矩阵为唯一裁判。
...
上一批让原生 `confirm` 归零，但面板里还剩**第二套形态**：三处用 `useState` 存起待确认对象、再由 `Dialog`
拼出的是/否框。同一件事两种长相，规范就还是散的。

- LanPage：`pendingOn`（`{title, desc, act}`）—— 一个 Dialog 同时服务「开启远程控制」与「切公网」，
  于是切公网的按钮也写着「确认开启」，指错了动作；迁到 `askOn`/`askWan` 各问各的，按钮文案改为
  「开启远程控制」/「切换到公网」。
- InstancesPage：`confirmId` 驱动「删除实例？」—— 顺带把不指名的标题改成「删除实例「名称」？」，删除后
  `dataPreserved` 的如实提示逻辑一字未动（它证的是后端保留数据目录这条安全语义，与弹窗形态无关）。
- OverviewPage：`confirmStopDsh` 驱动「停止 DSH？」—— 收进 `stopDsh()`，触发口从「先置状态、再由 Dialog
  决议」变成一次 `await`。

三处原先都是 `max-w-[380px]/[400px]/[420px]` 各写一遍的自己一套样式，现由确认层统一（`max-w-[420px]`）。
`tone` 保持原视觉：只有删实例原本用 `variant="destructive"`，故仅它标红；开远程/切公网/停主干按原样走默认
按钮——**没有借迁移偷偷改判定强度或颜色语义**。

门禁同步收紧（仍并入 U-d 段，不新增链条目）：
- 经统一出口的危险动作确认条数下限从 8 抬到 **11**（实测 12 处 = 8 处原生迁移 + 自拼收编出的 4 次询问：
  删实例、停主干 DSH、开远程、切公网——LanPage 原先一个共用 Dialog 服务两条开启路径，拆开后各问各的）；
- 两条新判据各咬一面：`<DialogTitle>` 不得出现疑问句（那是是/否确认的长相）；不得有 `confirm/pending/will`
  命名的 `useState` 去驱动 `<Dialog>`（那是自拼确认的状态）。配正反两组样本：`删除实例？` 与
  `const [confirmId, setConfirmId] = useState(null); <Dialog open={!!confirmId}>` 必须命中，
  `添加 DSH 实例` 与 `addOpen`/`tokenFor` 这类表单态不得误报；
- 这条反向样本首跑即判红，暴露的是它自己：两半合成一个 `&&`，其中状态面写成 `set\1` 回引，回引拿到的是
  小写 `confirmId` 而源码 setter 是 `setConfirmId` —— 正则永不匹配，于是它旁边那条「不误报」判据一直恒真。
  改为**每条反向样本单独断言**，命名比对移到 JS 里做（取两个捕获组再比首字母大写）。反向样本与它保护的判据
  必须能各自独立变红，否则坏掉的判据会连带把自己的自测一起作废；
- 内核侧 `round13-csp-probe` G 段两条原判据钉的是旧形态（`setPendingOn` 装载数、`Dialog open={confirmStopDsh}`），
  改为锁「该高危动作是否各经一次统一确认出口」，并同批拒绝旧形态残留——**换实现路径判据要跟着换锚点，
  不能留着让闸去锁一个已经不存在的写法**。

**边界如实登记**（`ui/FRAMEWORK.md` 4.5）：两条新判据认的是「自拼确认的形态特征」，不是「所有 Dialog 只用于
表单」这个全称命题——换个不像确认的变量名仍可躲过状态面，但标题面与出口计数面同时兜住大部分回归；真正
结构性的禁止需要组件级 DOM 测试设施（vitest 仍是 node 环境，无 jsdom）。观感与 Esc/取消分支的验证仍候发布后
真机走查。
...
把两件不相干的事绑在一起**，再叠上**流程没有闭环**：

- `domains/relay/core.js#projectRemoteView`（远程可用性的唯一投影）把「未设访问令牌」当成 `lan` 模式的
  未就绪原因。但 relay 门卫对空令牌**恒放行**（`tokenGateDecision`），局域网侧本就能直接访问——于是
  `ready:false`，面板据此不渲染二维码，用户看到的是「开着却什么也没有」。令牌只属于 `wan` 的闸
  （公网口无令牌等于 DSH 特权接口对公网零认证可达）。
- 即使判定修好，流程仍是半截：`wan` 有「先设令牌」的硬前置，而「开启远程控制」是用户唯一的开远程动作。
  把「先去别处设凭据」留在流程里，产出的就是开关已开、屏幕无入口、用户也不知凭据为何的状态。
- 附带病灶：视图未就绪时 `accessUrl` 也不给，用户在等待期/失败期连可复制的地址都没有。

三处各归其位（能力零损伤：`wan` 的强度闸、`/lan-access` 的脱敏、事件载荷的布尔化全部保持；
强度下限 8 位在两处执行口都照常生效——沙箱写口 `updateInstance` 先验强度再落盘，自动分配的值 16 位
URL-safe 串本就过闸，而 `updateInstance` 的「写入口即拒」继续作为第二道真实执行边界）：

- **判定层**：`projectRemoteView` 的令牌子句移入 `mode==='wan'` 分支；`accessUrl` 与 `ready` 解绑——
  端口与主机已定即给出，未就绪时地址行照旧呈现、`reasons` 另占一行。
- **写入层**：`setRemoteMode`（意图唯一写入口）在 `mode!==off` 且现值为空时，经**唯一分配口**
  `core.generateRemoteToken`（`crypto.randomBytes(12).base64url`，只出 URL-safe 字符，因门卫的一次性
  出示形态是 `?token=`）生成，并与模式**同一次落盘**；`wan` 前置闸按**补齐后**的令牌复判，被拒仍零写入。
  已有值（含过弱的历史值）一律不覆盖，弱令牌继续由闸显式拒绝。分配事实以 `tokenAutoAllocated` 回执给面板。
- **呈现边界**：`remoteToken` 明文只随 `/instances` 的**回环分支**下发（判据与 `authUrl` 的 `?token=`
  同源：`identity.loopback`，socket 层现取），面板的「设置令牌」框据此预填当前值、支持明文显隐与就地修改
  （值未变则不发写请求）；`/lan-access` 的白名单**一字未动**，仍零令牌字段。TK-5/TK-G6 未放宽。
- **扫码可用**：补齐令牌意味着门卫从此**要求**令牌（空令牌才恒放行），二维码若只编裸地址，扫出来就是一页
  401——那等于把「显示不出二维码」换成「二维码扫了没用」。故本机面板持有明文时，二维码编 `?token=<令牌>`
  的一次性出示（之后凭 `dsh_lan_token` Cookie）；访客面板读不到明文，码内只有裸地址，与今天一致。
  **地址行的点击口不带令牌**：TK-G6 禁令牌进浏览器启动参数，那里保留 401 提示页就是正解。

门禁（新增判据全部并入既有条链目）：`defects-batch-f` C-3b 逐子句改到新语义并加正向例（`lan` 缺令牌仍
`ready:true` 且出地址、`wan` 缺令牌必计因、未就绪仍出 `accessUrl`），反向 flip 从 3 条扩到 5 条
（基线改取 wan 全就绪，五个原因字段各翻一次）；`round13` ① 的行为例**改为每例一份新鲜夹具**（原实现
跨例共享写次数账本，判据实际锁的是执行顺序），新增自动分配三条（一次落盘携带两字段、令牌达下限且
URL-safe、事件载荷零明文），「被拒时未落盘」的判据换由**已有弱令牌**给出；`api-contract` F1 加回环下发
明文例、F2 加「已认证 LAN 访客只见布尔」例。契约表 `src/api/contract.js` 与 DSH-TOKEN-CONTRACT.md §5
的 TK-7 追加裁决同步成文。

一处门禁随之纠正：`round13-csp-probe` G 段原把令牌输入框的整串字面量（`type="password" autoComplete="new-password"
placeholder="输入访问令牌"`）钉死，显隐开关一上就判红。改为锁行为——令牌框必须默认遮罩、点眼睛才显明文，
并补反向样本（退回常显明文即转红），使这条闸认形态而非认文案。

**未收口**：真机（Windows 那台）的「开远程控制→出二维码→点钥匙看令牌→改令牌」全程证据仍待发布后取。
