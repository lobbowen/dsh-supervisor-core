...
(async () => {
  ...
  // -- MIG-6+：B14—— 解析损坏不崩 + 半途失败不双登记 --
  {
    // 1) 源文件损坏：返回 0、不抛、**不碰源文件**、不产出目标
    ...
    // 3) 目标损坏：不得被覆盖，且**绝不顺手清空源**（旧实现吞异常后照常清源 -> 两头无存）
    ...
    check('MIG-8 目标损坏：不抛、返回 0、源记录仍在、坏目标未被清写',
      ...
      'r=' + r2);
    // 4) 半途失败：目标文件已存在（走 targetExisted 分支）且其父目录被设成只读（0500）。
