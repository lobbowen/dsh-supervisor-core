...
//         latest 优先，latest 缺失/非法才回落 versions 最高
...
//   （BETA 的数字可能压过 RC，契约 已论证）。现收口为 dist/index.js 导出的
//   唯一实现 pickReleaseVersion；本门禁把 五步与「第三方不变」钉死。
// ---------------------------------------------------------------------------
...
// 选版算法是 dist 导出的**唯一实现**——本测试直接测它，不另建实现。
const channel = dist;
...
// -- 2)：灰度定向 --
{
  ...
  // -- 防回归：灰度判定**不得**以"名单包存在"为依据 --
  //   曾经的错误实现：`_inCanaryList()` 里"装了 @dsh-sup/canary-allowlist 就算命中"。
  //   那是"全员灰度"（该包为所有候选机共用），与"定向"的初衷相反。
  //   契约 已冻结正确语义（读**内容** + schema===1 + installId/hostnames 匹配）；
  //   且该包当前**未发布**（预留），故判定应恒为"非灰度"，只走本地开关。
  //  （域结构第三轮）：distribution 已拆分，按目录聚合读取。
...
// -- 4)：latest 缺失/非法 -> versions 最高--
{
  // 发布条 4改判说明：本例原期望 '0.1.5-BETA.7'（全量最高），
  //   即「镜像响应里 latest 缺失 -> 稳定版机器被静默升到测试版」。现按契约 4) 排除 BETA。
  ...
  // 3) 与 4) 的分工：latest **显式指向** BETA（当前线上实况）时照原样采纳，不受 发布条 4 影响。
...
// -- 5)：皆无 -> null（绝不猜）--
{
  ...
  // 发布条 4：4) 排除 BETA 后无候选 -> 落到 5) 明确失败（RC-5：如实报错，不得静默当成"已是最新"）。
