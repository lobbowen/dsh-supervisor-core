...
// 守卫永远无法自起 router/lan daemon。若既有测试（daemon-lifecycle-test.js、
// lan-daemon-test.js）**直接构造 / 自行 spawn** 而绕过这条路径，
// 就会出现「用例都过、生产路径无人验」的假门禁。
//
...
//   app/daemons/scripts.js（DIRECTORY-STRUCTURE-DESIGN 反转法）。srcpath 只留通用 resolve。
...
{
  //  步骤7：daemonScript 调用点（_daemonLifecycle）已从 control-view.js -> app/daemons/runtime.js。
  ...
  //  函数体内联 require 已上提为模块顶层 require。判据随之放宽为
