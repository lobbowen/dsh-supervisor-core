...
//  （域结构第三轮）：distribution 已拆分，按目录聚合读取（平台标签委托落在 registry.js）。
...
check('A-c 平台判定也不得静默回落 linux',
  ...
  '已移除');
// （跨平台架构规范化）：平台映射表已收口到 src/platform/contract/matrix.js，
//   故此处**不再断言 dist/index.js 里存在映射表**（那是实现细节，且正是被消除的重复），
//   改为断言两条**结构性不变量**：
//     1) dist 的 _platformTag 必须委托平台层（不得自己持平台知识）；
//     2) 平台层矩阵确实存在白名单映射与「未知组合抛错」。
...
function tag(platform, arch) {
  // 复（P1）：必须对插值做 JS 字符串转义（用 JSON.stringify）。
