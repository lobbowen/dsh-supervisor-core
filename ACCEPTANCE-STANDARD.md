...
以及同文件 `build` job 定义处的注释（**不记行号**：workflow 一改版行号就飘，此处此前记的
「139-142 行」实测已指向 precheck 的版本探测段，那段与被引内容无关）：
...
- ✅ **同口径只读复算**：读文件 + 跑门禁判据的纯函数部分（不执行被测代码），用于推送前预拆雷；
  它只是**预拆雷**，不是裁决，结论仍归 CI；
...
无控制台窗口 K-W2、有界执行 G9、平台可移植性 X-11、能力审计 A2/A3 等），其文件头必须含一个
...
- 标记本身是**契约字面量**：J-o 用字符串匹配找 `## 覆盖缺口（E-2 制度化登记`，
  精简注释、改标题措辞、批量替换符号时都必须保留该前缀原样；改前缀要同批改 J-o 与本文。
...
## 10. 判据自身有六条失效形态，写判据时必须先排掉（第 4 批 CI 取证，2026-09-20）

**规则**（取证链见 AUDIT-REPORT §H-7-8/9/11/12/13/14/15 与 §I-9；本条不新增机器执法，靠评审）：
...
5. **夹具重算计划要连输入一起重算**：产品的计划常来自运行期探测（旧形态里执行口内部还会自己
   `findChromeWin()`；现行形态是执行口只按环境表单的分发依据取 `plan.bin`，本机实况一律来自
   `platform/os/environment.js`），夹具只重算纯函数那一半就会**不同源**——本仓实测：夹具认定
...
6. **判据的「对象」可能是注释措辞**：以字面量 grep 注释/文档的门禁（正则、`indexOf()`、`includes()`），
   在批量精简注释、替换符号字符、改标题措辞时**静默失配**——门禁的名字没变、判据的逻辑没变，
   红绿却由一处措辞决定。本仓实测：一次注释精简把 `## 覆盖缺口（E-2 制度化登记` 的括号部分删掉，
   J-o 的字符串匹配落空 ⇒ 四平台同时判红（且 `&&` 链在其后截断，掩盖了未执行的那一段）。
   规则：被机器 grep 的注释标记是**契约字面量**（见 §7 边界），批量改写前先 grep 它的全部引用点；
   要改标记，就在同一次改动里连执法点与规范正文一起改。

**为什么这六条同属一类**：它们都不会让产品变坏，只会让**门禁在绿与红之间撒谎**——
恒空的段、恒 0 的计数、恒空的异步事件、不成立的环境前提、不同源的夹具计划、失配的注释标记，
判红时报的都是「产品没做」，读者却要去改产品。
而代价不对称：**崩溃 > 判红 > 空转**（判红吃掉一条断言，崩溃吃掉整份文件加链上其后所有文件，
空转则把缺口永久隐藏），所以异步块要把汇总与 `process.exit` 挂到完成回调上，
并让块自身抛错降级为一条 FAIL。

**边界**：本条不豁免 §0/§5 的限制——上述六点都可以在本机用**只读静态审阅**或
