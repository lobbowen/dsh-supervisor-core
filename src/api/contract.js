...
const SURFACE = [
  ...
  { path: '/env/environment',      methods: ['GET'],  domain: 'guard', category: 'public',      consumers: ['UI(OverviewPage 环境表单)', 'README 文档化（人工/支持排障直读；打开动作的诊断走 evidence.diagnostics）'], note: '环境表单（只读装配面）：候选浏览器 + 系统默认及其来源 + 图形会话 + 能力档位 + 用户偏好 + 这一拍的分发依据 + 每条系统查询留痕；?force=1 重探、?persist=1 落快照' },
