...
// 与 `shared/ip`（来源可信判定被 relay 与 api 共用）同形，按 DIRECTORY-STRUCTURE-DESIGN 的
// 既有裁决上移到 shared，单一事实源在此，绝不重写第二份。

/** 远程访问令牌强度闸：与 apiAccessKey 的「至少 8 位」门（app/settings/access.js）
 *  同规。旧闸只判「非空白」——remoteToken 守护的是经 frp 暴露到公网的 DSH 特权面
 *  （relay 空 token 恒放行 + 回环呈现），1~2 位令牌等同无令牌，可被公网暴力枚举。
 *  纯函数：只回判定，落点（写盘前 / 暴露闸）由调用方负责。
 *  @param {string} token  @returns {{ok:boolean, reason:string}} reason 属于 ''（合格）| 'empty' | 'short' */
